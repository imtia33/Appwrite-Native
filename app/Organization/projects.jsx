...
const Projects = () => {
  ...
  const getUniquePlatforms = (platforms = []) => {
    ...
    platforms.forEach(platform => {
      ...
      if (type.includes('flutter')) {
        ...
        detectedType = 'react';
      } else if (name.toLowerCase().includes('javascript') || name.toLowerCase().includes('js')) {
        detectedType = 'js';
  ...
  const getPlatformIcon = (detectedType) => {
    switch (detectedType) {
      case 'flutter': return { component: FontAwesome6, name: 'flutter' };
      case 'react':
      case 'react-native': return { component: MaterialCommunityIcons, name: 'react' };
      case 'js': return { component: MaterialCommunityIcons, name: 'language-javascript' };
      case 'android': return { component: MaterialCommunityIcons, name: 'android' };
      case 'ios': return { component: MaterialCommunityIcons, name: 'apple' };
      default: return { component: MaterialCommunityIcons, name: 'web' };
    }
  };
    ...
    <View className="flex-1 bg-background">
      <View className="flex-row items-center gap-3 p-4">
        ...
        <Button className="rounded-xl px-2 h-10 flex-row items-center gap-1">
          ...
          <Text style={{ fontSize: 16, marginBottom: 2 }} className="text-primary-foreground font-regular">
            Create Project
          </Text>
        </Button>
      </View>
      ...
      <ScrollView className="flex-1 px-2">
        {loadingProjects ? (
          ...
          filteredProjects.map((proj) => {
            ...
            return (
              <Card key={proj.$id} className="bg-card border-border shadow-none rounded-2xl mb-4">
                <View className="px-4 py-3">
                  <Text className="text-muted-foreground text-sm font-regular mb-1">
                    {proj.platforms?.length || 0} apps
                  </Text>
                  <Text style={{ marginBottom: 80 }} className="text-foreground text-2xl font-regular">
                    {proj.name}
                  </Text>
                  ...
                  <View className="flex-row items-center justify-between">
                    <View className="flex-row gap-2 flex-wrap flex-1">
                      {platformTypes.slice(0, 3).map((type, idx) => {
                        ...
                        return (
                          <Badge key={idx} variant="secondary" className="px-3 py-2 rounded-xl border-0">
                            <IconComponent
                              name={iconData.name}
                              size={16}
                              color="#969696"
