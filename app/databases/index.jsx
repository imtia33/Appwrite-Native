...
import { View, Text, Animated, Dimensions, TouchableOpacity, StyleSheet, Easing, InteractionManager, ActivityIndicator } from 'react-native';
...
const DatabaseLayout = () => {
    ...
    useEffect(() => {
        if (activeDatabaseId && currentProject?.$id) {
            fetchCollections(currentProject.$id, currentProject.region || 'fra', activeDatabaseId);
        }
    }, [activeDatabaseId, currentProject?.$id]);

    const openTableScreens = useCallback((collectionId) => {
    ...
    const handleDatabaseChange = useCallback((dbId) => {
    ...
    const activeDatabase = useMemo(() => databases.find(db => db.$id === activeDatabaseId), [databases, activeDatabaseId]);
    const activeCollections = useMemo(() => activeDatabaseId ? (collections[activeDatabaseId] || []) : [], [collections, activeDatabaseId]);
    const activeCollection = useMemo(() => activeCollections.find(c => c.$id === activeCollectionId), [activeCollections, activeCollectionId]);
            ...
            <View className="flex-1 flex-row relative overflow-hidden">
                ...
                <View className="flex-row items-center gap-1">
                    <TouchableOpacity onPress={() => router.back()} className="mr-2">
                        <ChevronLeft size={20} color={theme === 'dark' ? '#FFFFFF' : '#000000'} />
                    </TouchableOpacity>
                    <OrganizationPicker 
