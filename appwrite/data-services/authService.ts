...
const useAuthStore = create<AuthStore>((set, get) => ({
    ...
    fetchUsers: async (projectId: string) => {
        ...
        if (state.loading.users || (state.currentProjectId === projectId && state.users.length > 0)) {
            return;
        }
        ...
        try {
            ...
            const response = await sdk.forProject(currentProject.region || 'fra', currentProject.$id).users.list();
    ...
    fetchTeams: async (projectId: string) => {
        ...
        // Return early if already loading or data exists for same project
        if (state.loading.teams || (state.currentProjectId === projectId && state.teams.length > 0)) {
            return;
        }
