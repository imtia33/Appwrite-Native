...
interface BucketActions {
    ...
    createBucket: (projectId: string, region: string, name: string, bucketId?: string) => Promise<any>;
    updateBucket: (projectId: string, region: string, bucketId: string, name: string) => Promise<any>;
    deleteBucket: (projectId: string, region: string, bucketId: string) => Promise<void>;
}
...
const useBucketStore = create<BucketStore>((set, get) => ({
    ...
    fetchBuckets: async (projectId: string) => {
        ...
        if (state.loading || (state.currentProjectId === projectId && state.buckets.length > 0)) {
            return;
        }
        ...
        try {
            ...
            const response = await sdk.forProject(currentProject.region || 'fra', currentProject.$id).storage.listBuckets();
    ...
    createBucket: async (projectId: string, region: string, name: string, bucketId: string = 'unique()') => {
        try {
            const response = await sdk.forProject(region, projectId).storage.createBucket(
                bucketId,
                name,
    ...
    updateBucket: async (projectId: string, region: string, bucketId: string, name: string) => {
        try {
            const response = await sdk.forProject(region, projectId).storage.updateBucket(
                bucketId,
                name
            );
