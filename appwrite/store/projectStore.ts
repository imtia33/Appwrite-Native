...
export const useProjectStore = create<ProjectState>((set) => ({
  ...
  fetchPlatforms: async (projectId: string) => {
    ...
    try {
      ...
      set((state) => ({
        loadingSub: { ...state.loadingSub, platforms: false },
      }));
    }
  },
  ...
  fetchApiKeys: async (projectId: string) => {
    ...
    try {
      const response: any = await sdk.forConsole.projects.listKeys({
        projectId,
      });
  ...
  fetchDevKeys: async (projectId: string) => {
    ...
    try {
      const response: any = await sdk.forConsole.projects.listDevKeys({
        projectId,
      });
  ...
  setCurrentProject: async (project) => {
    ...
    if (project) {
      AsyncStorage.setItem("selectedProjectId", project.$id);
      AsyncStorage.setItem("last_view", "project");
    } else {
  ...
  deleteDevKey: async (projectId: string, keyId: string) => {
    try {
      ...
      set((state) => ({
        devKeys: state.devKeys.filter((k) => k.$id !== keyId),
      }));
    } catch (error) {
