...
import { View, Text, ScrollView, TouchableOpacity, Switch, ActivityIndicator, Alert, ToastAndroid, Platform } from 'react-native';
...
import { Shield, Users, Clock, History, AlertTriangle, ShieldCheck, Mail, Info } from 'lucide-react-native';
...
const SecurityScreen = () => {
    ...
    useEffect(() => {
        if (currentProject?.$id) {
            // Auth Limit
            setAuthLimitValue(currentProject.authLimit !== 0 ? 'limited' : 'unlimited');
    ...
    const updateAuthLimit = () => {
        if (!sdk || !sdk.forConsole || !currentProject?.$id) {
            ...
            return Promise.reject(new Error("SDK not initialized or project not selected"));
        }
    ...
    if (!currentProject?.$id) {
        return (
            <View className="flex-1 items-center justify-center p-4">
                <Text className="text-muted-foreground">Please select a project first</Text>
            </View>
        );
    }
    ...
    return (
        <ScrollView className="flex-1 bg-background" contentContainerStyle={{ padding: 16, paddingBottom: 40 }}>
            <View className="mb-6">
                ...
                <Text className="text-muted-foreground text-sm">Configure your project authentication security settings</Text>
            </View>
            ...
            <Card className="mb-6 border-border  py-4">
                <CardHeader>
                    ...
                    <CardDescription>
                        Limit new users from signing up for your project. You can still create users manually from the console.
                    </CardDescription>
                </CardHeader>
                    ...
                    {authLimitValue === 'limited' && (
                        <View className="flex-row items-center gap-2 mt-2">
                            <Input 
                                ...
                                onChangeText={text => setAuthLimit(text.replace(/[^0-9]/g, ''))}
                            />
                            <Text className="text-muted-foreground text-sm">Maximum users</Text>
                        </View>
                    )}
                </CardContent>
                <CardFooter className="border-t border-border pt-4">
                    <Button 
                        ...
                        size="sm"
                    >
                        {updating.authLimit ? <ActivityIndicator size="small" color="white" /> : <Text className='text-white font-medium'>Update</Text>}
                    </Button>
                </CardFooter>
            </Card>
            ...
            <Card className="mb-6 border-border  py-4">
                <CardHeader>
                    ...
                    <CardDescription>
                        Set how long a user session remains active. Reducing this will log out active users.
                    </CardDescription>
                </CardHeader>
                <CardContent className="flex-row gap-4 items-end">
                    <View className="flex-1 gap-2">
                        ...
                        <Input 
                            ...
                            onChangeText={text => setSessionLength(text.replace(/[^0-9]/g, ''))}
                        />
                    </View>
                    <View className="flex-1 gap-2">
                        ...
                        <Select value={sessionUnit} onValueChange={setSessionUnit}>
                            ...
                            <SelectContent>
                                ...
                                <SelectItem label="Days" value="days" />
                                <SelectItem label="Months" value="months" />
                                <SelectItem label="Years" value="years" />
                    </SelectContent>
                        </Select>
                    </View>
                </CardContent>
                <CardFooter className="border-t border-border pt-4">
                    <Button 
                        ...
                        size="sm"
                    >
                        {updating.sessionLength ? <ActivityIndicator size="small" color="white" /> : <Text className='text-white font-medium'>Update</Text>}
                    </Button>
                </CardFooter>
            </Card>
            ...
            <Card className="mb-6 border-border  py-4">
                ...
                <CardContent className="gap-2">
                    ...
                    <Input 
                        ...
                        onChangeText={text => setSessionsLimit(text.replace(/[^0-9]/g, ''))}
                    />
                    <Text className="text-muted-foreground text-xs italic mt-1">Default is 10. Set to 0 for unlimited.</Text>
                </CardContent>
                <CardFooter className="border-t border-border pt-4">
                    <Button 
                        ...
                        size="sm"
                    >
                        {updating.sessionsLimit ? <ActivityIndicator size="small" color="white" /> : <Text className='text-white font-medium'>Update</Text>}
                    </Button>
                </CardFooter>
            </Card>
            ...
            <Card className="mb-6 border-border  py-4 ">
                <CardHeader>
                    ...
                    <CardDescription>
                        Set rules and requirements for user passwords to ensure account security.
                    </CardDescription>
                </CardHeader>
                <CardContent className="gap-6">
                    <View className="flex-row items-center justify-between">
                        <View className="flex-1 mr-4">
                            <Text className="text-foreground font-medium">Password history</Text>
                            <Text className="text-muted-foreground text-xs">Prevent users from reusing recent passwords.</Text>
                        </View>
                        <Switch 
                    ...
                    {passwordHistoryEnabled && (
                        <View className="flex-row items-center gap-2 ml-4">
                            <Input 
                                ...
                                onChangeText={text => setPasswordHistory(text.replace(/[^0-9]/g, ''))}
                            />
                            <Text className="text-muted-foreground text-sm">Previous passwords</Text>
                        </View>
                    )}

                    <View className="flex-row items-center justify-between">
                        <View className="flex-1 mr-4">
                            <Text className="text-foreground font-medium">Password dictionary</Text>
                            <Text className="text-muted-foreground text-xs">Prevent common or insecure passwords using a blacklist of 10k common passwords.</Text>
                        </View>
                        <Switch 
                    ...
                    <View className="flex-row items-center justify-between">
                        <View className="flex-1 mr-4">
                            <Text className="text-foreground font-medium">Disallow personal data</Text>
                            <Text className="text-muted-foreground text-xs">Don't allow passwords that contain the user's name, email, or phone.</Text>
                        </View>
                        <Switch 
                ...
                <CardFooter className="border-t border-border pt-4">
                    <Button 
                        ...
                        size="sm"
                    >
                        {updating.passwordPolicies ? <ActivityIndicator size="small" color="white" /> : <Text className='text-white font-medium'>Update</Text>}
                    </Button>
                </CardFooter>
            </Card>
            ...
            <Card className="mb-6 border-border py-4 ">
                ...
                <CardContent className="gap-6">
                    <View className="flex-row items-center justify-between">
                        <View className="flex-1 mr-4">
                            <Text className="text-foreground font-medium">Session alerts</Text>
                            <Text className="text-muted-foreground text-xs">Email users when a new session is created on their account.</Text>
                        </View>
                        <Switch 
                            value={sessionAlerts}
                    ...
                    <View className="flex-row items-center justify-between">
                        <View className="flex-1 mr-4">
                            <Text className="text-foreground font-medium">Invalidate sessions</Text>
                            <Text className="text-muted-foreground text-xs">Clear all existing sessions when a user changes their password.</Text>
                        </View>
                        <Switch 
                ...
                <CardFooter className="border-t border-border pt-4">
                    <Button 
                        ...
                        size="sm"
                    >
                        {updating.sessionSecurity ? <ActivityIndicator size="small" color="white" /> : <Text className='text-white font-medium'>Update</Text>}
                    </Button>
                </CardFooter>
            </Card>
        </ScrollView>
