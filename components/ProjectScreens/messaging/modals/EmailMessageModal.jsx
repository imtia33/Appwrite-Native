...
import { X, Mail, Bell, Clock, Hash } from 'lucide-react-native';
...
const EmailMessageModal = ({ visible, onClose, onCreate, isLoading }) => {
            ...
            case 1:
                return (
                    <View className="gap-4">
                        ...
                        <View>
                            <Text className="text-sm font-medium text-foreground mb-1.5">Content (Plain text/HTML)</Text>
                            <TextInput
                                className="bg-input text-foreground px-4 py-3 rounded-xl border border-border h-32 text-top"
                        ...
                        <View className="flex-row items-center justify-between bg-muted/30 p-3 rounded-xl">
                            <View>
                                <Text className="text-sm font-semibold text-foreground">HTML Message</Text>
                                <Text className="text-xs text-muted-foreground">Treat content as HTML</Text>
                            </View>
                            <Switch 
                                value={formData.html} 
                                onValueChange={(val) => setFormData({ ...formData, html: val })}
                ...
                <View className="bg-card rounded-t-[32px] border-t border-border max-h-[90%]">
                    <View className="p-6 border-b border-border flex-row justify-between items-center">
                        <View className="flex-row items-center gap-3">
                            ...
                            <Text className="text-xl font-bold text-foreground">Create Email</Text>
                        </View>
                    ...
                    <View className="p-6 bg-muted/20 border-t border-border flex-row gap-3">
                        ...
                        {step < 3 ? (
                            <TouchableOpacity 
                                ...
                                disabled={step === 1 && (!formData.subject || !formData.content)}
                                className={cn("flex-1 bg-primary py-4 rounded-2xl items-center", (step === 1 && (!formData.subject || !formData.content)) && "opacity-50")}
                            >
                            ...
                            >
                                {isLoading ? <ActivityIndicator color="white" size="small" /> : <Text className="text-white font-bold">Create Email</Text>}
                            </TouchableOpacity>
                        )}
                    </View>
                </View>
