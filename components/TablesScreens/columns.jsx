...
const Columns = ({ databaseId, tableId }) => {
  ...
  const handleCreateAttribute = async ({ type, data }) => {
    ...
    try {
      ...
      Alert.alert("Error", err.message || "Failed to create column");
    } finally {
  ...
  const tableColumns = [
    {
      ...
      cell: ({ row }) => (
        <View className="flex-row items-center">
          ...
          <Text className="text-muted-foreground font-medium text-[15px] ml-1 ">
            {row.original.key}
          </Text>
          {row.original?.array && (
            <MaterialIcons name="data-array" size={18} color="gray" />
          )}
          {row.original?.encrypt && (
            <FontAwesome
              name="lock"
              size={18}
              color="gray"
          ...
          {row.original.required && (
            <Text className="text-muted-foreground ml-1 px-2 py-1 bg-input rounded-sm">
              required
            </Text>
          )}
        </View>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <View className="">
          <Text className="text-xs text-muted-foreground uppercase text-center">
            {row.original.type}
          </Text>
        </View>
      ),
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <TouchableOpacity
          onPress={() => handleDeleteAttribute(row.original)}
    ...
    <View className="flex-1 bg-background">
      <View className="px-4 py-2 border-b border-border flex-row items-center justify-between">
        ...
        >
          ...
          <Text className="text-white font-bold ml-1">Column</Text>
        </Button>
      </View>
      ...
      {columns.length === 0 && !loading ? (
        <View className="flex-1 items-center justify-center p-8">
          ...
          <Text className="text-muted-foreground text-center mt-2">
            Create columns to define the structure of your rows.
          </Text>
        </View>
      ) : (
        <DataTable
