...
const Security = ({ databaseId, tableId }) => {
  ...
  useEffect(() => {
    if (table) {
      setRowSecurity(table.documentSecurity || false);
      setIsLoading(false);
    }
  }, [table]);
  ...
  useEffect(() => {
    if (table) {
      setHasChanges(rowSecurity !== (table.documentSecurity || false));
    }
  }, [rowSecurity, table]);
    ...
    <>
      <Card className="p-4 mb-4">
        <View className="flex-row items-center justify-between mb-4">
          ...
          <Switch checked={rowSecurity} onCheckedChange={setRowSecurity} />
        </View>
        <Text className="text-sm text-muted-foreground leading-relaxed">
          When row security is{" "}
          <Text className="font-bold text-foreground">enabled</Text>, users will
          be able to access rows for which they have been granted either row or
          table permissions.
        </Text>
        <Text className="text-sm text-muted-foreground mt-2 leading-relaxed">
          If <Text className="font-bold text-foreground">disabled</Text>, users
          can access rows only if they have table permissions. Row permissions
          will be ignored.
        </Text>
      </Card>
