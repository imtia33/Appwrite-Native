...
const Indexes = ({ databaseId, tableId }) => {
  const { currentProject } = useProjectStore();
  const { fetchIndexes, fetchColumns, deleteIndex, createIndex } =
    useDatabaseStore();
  ...
  }, [currentProject?.$id, databaseId, tableId]);
  ...
  const columns = [
      ...
      cell: ({ row }) => (
        <View className="flex-row flex-wrap gap-1">
          {row.original.attributes.map((attr, i) => (
            <View
              key={i}
              className="bg-primary/5 px-2 py-0.5 rounded border border-primary/10"
            >
              <Text className="text-[10px] text-primary">
                {attr} ({row.original.orders[i]})
              </Text>
            </View>
          ))}
        </View>
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <TouchableOpacity
          onPress={() => handleDeleteIndex(row.original)}
