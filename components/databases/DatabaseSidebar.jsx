...
const DatabaseSidebar = React.memo(
  ({ databases, activeDatabaseId, onDatabaseChange }) => {
          ...
          {databases.map((db) => {
            ...
            return (
              <View key={db.$id} className="relative items-center">
                {/* Indicator line */}
                {isActive && (
                  <View className="absolute left-[-16px] top-[12px] w-[4px] h-[24px] bg-primary rounded-r-full" />
                )}
