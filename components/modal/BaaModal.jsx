...
const BaaModal = ({ open, onOpenChange, locale, countryList }) => {
    ...
    const handleSubmit = async () => {
        setLoading(true);
        try {
            // Replicating the Svelte fetch logic
            const response = await fetch('https://growth.appwrite.io/v1/support', {
                ...
                body: JSON.stringify({
                    ...
                    message: `BAA request for ${currentOrganization?.name ?? ''} (${currentOrganization?.$id ?? ''})`,
                    tags: ['cloud'],
                    customFields: [
                        { id: '41612', value: 'BAA' },
            ...
            if (response.ok) {
                onOpenChange(false);
                // Add notification logic if available globaly
            }
        } catch (error) {
            console.error(error);
        } finally {
    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-[425px]">
                <DialogHeader>
                    <DialogTitle>Request BAA</DialogTitle>
                </DialogHeader>
